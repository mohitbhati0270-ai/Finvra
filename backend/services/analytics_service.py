...
def compute_portfolio_analytics(
    ...
    port_return   = float(np.dot(w, ann_returns))
    port_variance = float(np.dot(w, np.dot(ann_cov, w)))
...
def compute_efficient_frontier(
    ...
    def portfolio_vol(w):
        return float(np.sqrt(np.dot(w, np.dot(cov_matrix, w))))
    ...
    return {
        ...
        "min_variance": {
            "risk":   round(min_vol * 100, 2),
            "return": round(min_ret * 100, 2),
        },
        "max_sharpe": {
...
def generate_insights(
    ...
    if benchmark_return is not None:
        ...
        if diff > 0:
            insights.append(
                f"Your portfolio returned {port_return:+.2f}% vs NIFTY 50's "
                f"{benchmark_return:+.2f}% — you are outperforming the market "
                f"by {abs(diff):.2f}%. Keep it up!"
            )
        else:
            insights.append(
                f"Your portfolio returned {port_return:+.2f}% vs NIFTY 50's "
                f"{benchmark_return:+.2f}% — you are underperforming the market "
                f"by {abs(diff):.2f}%. Consider rebalancing."
            )
    else:
        if port_return > 0:
            insights.append(
                f"Your portfolio has generated a positive return of "
                f"{port_return:+.2f}% annually."
            )
        else:
            insights.append(
                f"Your portfolio is down {port_return:.2f}% annually. "
    ...
    if sharpe >= 1.0:
        insights.append(
            ...
            f"justify the risk taken. This is considered a strong portfolio."
        )
    elif sharpe >= 0.5:
        insights.append(
            f"Good Sharpe Ratio of {sharpe:.2f} — your returns reasonably "
            f"justify the risk. Room for improvement through diversification."
        )
    elif sharpe >= 0:
        insights.append(
            f"Low Sharpe Ratio of {sharpe:.2f} — returns barely justify "
            f"the risk. Consider adding lower-risk stocks to improve this."
        )
    else:
        insights.append(
            f"Negative Sharpe Ratio of {sharpe:.2f} — a fixed deposit at "
            f"6.5% is currently beating your portfolio on a risk-adjusted basis. "
    ...
    top_risk_stock = sorted_by_risk[0]
    if top_risk_stock["risk_contribution_pct"] > 35:
        insights.append(
            f"{top_risk_stock['ticker']} contributes {top_risk_stock['risk_contribution_pct']:.1f}% "
            f"of your total portfolio risk despite only {top_risk_stock['weight']:.1f}% weight — "
            f"this is high concentration risk. Consider reducing it below 25%."
        )
    elif top_risk_stock["risk_contribution_pct"] > 25:
        insights.append(
            f"{top_risk_stock['ticker']} is your biggest risk driver at "
            f"{top_risk_stock['risk_contribution_pct']:.1f}% risk contribution. "
            f"Monitor this position closely."
        )
    else:
        insights.append(
            f"Your portfolio is well diversified — no single stock dominates "
            f"risk. {top_risk_stock['ticker']} is the largest contributor at "
    ...
    if beta > 1.2:
        insights.append(
            ...
            f"{((beta-1)*100):.0f}% more volatile than NIFTY 50. "
            f"You will gain more in bull markets but lose more in crashes."
        )
    elif beta > 1.0:
        insights.append(
            f"Portfolio beta of {beta:.2f} — slightly more aggressive than "
            f"NIFTY 50. Good for bull markets but adds downside risk."
        )
    elif beta > 0.8:
        insights.append(
            f"Portfolio beta of {beta:.2f} — slightly defensive. Your portfolio "
            f"moves a little less than the market, offering some downside protection."
        )
    else:
        insights.append(
            f"Low beta of {beta:.2f} — your portfolio is quite defensive and "
            f"will be less affected by market crashes. Good for conservative investors."
        )
