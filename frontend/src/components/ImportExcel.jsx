...
export default function ImportExcel({ onImport }) {
  ...
  const handleFile = (e) => {
    ...
    reader.onload = (evt) => {
      try {
        ...
        if (headers.length === 0) { setError('Could not find stock/symbol column. Make sure Row 1 has headers: Stock Name, Qty, Avg Buy Price'); return }
        ...
        for (let i = headerRowIdx + 1; i < rows.length; i++) {
          ...
          if (!raw || isNaN(qty) || isNaN(price) || qty <= 0 || price <= 0) continue
          const ticker = resolveTickerName(raw)
          if (!ticker) continue
          holdings.push({ ticker, qty, avgPrice: parseFloat(price.toFixed(2)), _original: raw })
        }

        if (holdings.length === 0) { setError('No valid holdings found.'); return }
        setPreview(holdings)
        setParsed(holdings)
  ...
  return (
    <div>
      ...
      {error && (
        <div style={{ marginTop: '12px', padding: '12px 16px', background: 'rgba(239,68,68,0.1)', border: '1px solid rgba(239,68,68,0.3)', borderRadius: '10px', color: '#EF4444', fontSize: '13px' }}>
          ⚠️ {error}
      ...
      {showGuide && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.6)', zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
          <div style={{ background: theme.navyCard, border: `1px solid ${theme.border}`, borderRadius: '20px', padding: '32px', maxWidth: '520px', width: '100%', boxShadow: '0 24px 64px rgba(0,0,0,0.4)' }}>

            {/* Header */}
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '24px' }}>
              <div>
                <h3 style={{ margin: 0, fontSize: '18px', fontWeight: '800', color: theme.white }}>📊 Excel Format Guide</h3>
                <p style={{ margin: '6px 0 0', fontSize: '13px', color: theme.gray }}>Your Excel file must follow this structure</p>
              </div>
          ...
          <div style={{ background: theme.navyCard, border: `1px solid ${theme.border}`, borderRadius: '20px', padding: '28px', maxWidth: '600px', width: '100%', maxHeight: '80vh', overflowY: 'auto', boxShadow: '0 20px 60px rgba(0,0,0,0.4)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
              <div>
                ...
                <p style={{ margin: '4px 0 0', fontSize: '13px', color: theme.gray }}>Confirm the tickers look correct before importing</p>
              </div>
